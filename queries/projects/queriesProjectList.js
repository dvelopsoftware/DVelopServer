...
function MethodDBTeam() {
    ...
    this.select = function(respond) {
        connection.get(function(er, cn) {
            cn.query('SELECT * FROM projects_list', function(error, result) {
                cn.release();
                if (error) {
    ...
    this.selectId = function(id, respond) {
        connection.get(function(er, cn) {
            cn.query('SELECT * FROM projects_list WHERE id=?', id, function(error, result) {
                cn.release();
                if (error) {
                    respond.send({ status: 'Error' });
                } else {
    ...
    this.insert = function(data, respond) {
        connection.get(function(er, cn) {
            cn.query('INSERT INTO projects_list SET ?', data, function(error, result) {
                cn.release();
                if (error) {
                    respond.send({ status: 'Error' });
                } else {
    ...
    this.update = function(data, respond) {
        connection.get(function(er, cn) {
            cn.query('UPDATE projects_list SET ? WHERE id = ?', [data, data.id], function(error, result) {
                cn.release();
                if (error) {
                    respond.send({ status: 'Error' });
                } else {
    ...
    this.delete = function(id, respond) {
        connection.get(function(er, cn) {
            cn.query('DELETE FROM projects_list WHERE id = ?', id, function(error, result) {
                cn.release();
                if (error) {
                    respond.send({ status: 'Error' });
                } else {
